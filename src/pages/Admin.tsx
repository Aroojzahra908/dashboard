...
const Admin: React.FC = () => {
  ...
  const pendingContactsCount = useMemo(() => (contacts || []).filter((c: any) => c?.status !== "selected").length, [contacts]);
  const pendingApplicationsCount = useMemo(() => (applications || []).filter((a: any) => a?.status !== "selected").length, [applications]);
  ...
  async function handleApprove(table: string, id: string | number) {
    try {
      ...
      const currentStatus = record?.status === "selected" ? "selected" : "pending";
      const nextStatus = currentStatus === "selected" ? "pending" : "selected";

      // perform the DB update
      await supabase.updateRow(table, id, { status: nextStatus });
      ...
      if (nextStatus === "selected" && record) {
        // add to selected_students
        try {
          const payload = {
            source_table: table,
            source_id: record.id,
        ...
        // remove from selected_students when reverting back to pending
        try {
          // @ts-ignore custom helper exists
          await (supabase as any).deleteWhere("selected_students", { source_table: table, source_id: id });
          toast({ title: "Reverted", description: "Record set back to Pending." });
        } catch (delErr: any) {
          console.warn("Failed to clean selected_students on revert", delErr);
          toast({ title: "Updated", description: "Status set to Pending." });
        }
      } else {
        toast({ title: "Updated", description: `Status set to ${nextStatus}.` });
      }
    } catch (err: any) {
      console.error("Approve failed", err);
      toast({ title: "Error", description: err?.message || "Failed to update status. Ensure the table has a 'status' column and RLS allows updates." });
    }
  }
            ...
            <tbody>
              {merged.map((row: any) => (
                <tr key={`${row._source}-${row.id}`} style={{ borderBottom: `1px solid ${secondaryTint(0.35)}` }}>
                  ...
                  <td className="px-6 py-4">
                    <Button onClick={async () => await handleDelete(row._source === 'applications' ? 'job_applications' : 'contact_messages', row.id)} className="w-20 h-8 rounded-full text-xs font-semibold shadow-sm flex items-center justify-center" style={{ background: '#ef4444', color: '#fff' }}>
                      Delete
                    </Button>
                  </td>
                </tr>
            ...
            <div className="flex-1">
              ...
              <div className="mt-3 flex flex-wrap gap-3">
                <span className="inline-flex items-center rounded-full px-3 py-1 text-sm font-semibold" style={{ background: colors.primaryHex, color: colors.white }}>
                  {pendingContactsCount} Pending Leads
                </span>
                <span className="inline-flex items-center rounded-full px-3 py-1 text-sm font-semibold" style={{ background: colors.white, border: `1px solid ${primaryTint(0.06)}`, color: colors.secondaryHex }}>
                  {pendingApplicationsCount} Pending Applicants
                </span>
              </div>
            </div>
